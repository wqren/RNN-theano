...
from RNN_theano.datasets.forget_state import forget_state
...
def jobman(_options, channel = None):
    ...
    o = parse_input_arguments(_options, 'RNN_forget.ini')
    ####################### DEFINE THE TASK #########################

    rng = numpy.random.RandomState(o['seed'])
    train_set = forget_state(
                    n_states          = o['n_states']
                    , T             = o['task_T']
                    , switches      = o['task_switches']
    ...
    def wiener_hopf_fn( u_t, t_t, H_tm1, Y_tm1, W_hh, W_ux, b, h0):
        def recurrent_fn(u_t, lt_t, h_tm1, H_tm1, Y_tm1, W_hh, W_ux, b):
            x_t = TT.dot(W_ux, u_t)
            h_t = TT.tanh( TT.dot(W_hh, h_tm1) + x_t + b)
            H_t = H_tm1 + TT.dot(h_t, h_t.T)
            Y_t = Y_tm1 + TT.dot(h_t, lt_t.T)
            return h_t, H_t, Y_t
        [h_t, H_t, Y_t], _ = theano.scan(
            recurrent_fn
            , sequences = [u_t,t_t]
            , outputs_info = [h0 , H_tm1, Y_tm1]
